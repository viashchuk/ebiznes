<template>
    <div class="w-screen h-screen bg-white flex items-center justify-center">
        <div class="bg-white overflow-hidden rounded-lg border border-[#e5e7eb] w-[440px] shadow-sm">
            ...
            <div class="p-6 h-[474px] overflow-y-auto" ref="chatWindow">
                <Message v-for="(msg, index) in messages" :key="index" :role="msg.role" :content="msg.message" />
                <div v-if="isTyping" class="flex gap-3 my-4 text-gray-400 text-sm animate-pulse">
                    <div
                        class="shrink-0 w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full border border-gray-900">
